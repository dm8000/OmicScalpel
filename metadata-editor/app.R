...
#      actionButton("addCol", "Add Column", icon = icon("plus")),
      uiOutput("existing_columns_ui"),
      checkboxInput("toggle_na", "Hide Columns with only 'NA'", value = TRUE),
      tags$label(
...
"
      ...
      fileInput("upload_file", NULL, accept = ".txt"),
#      tags$div(style = "margin-top:15px; margin-bottom:5px;", tags$strong("Build metadata table")),
#      actionButton("addMeta", "Build"),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (is.null(rv$current_dataset) && length(rv$dataset_list) > 0) {
      rv$current_dataset <- rv$dataset_list[1]
    }
  ...
  output$dataset_selector <- renderUI({
    req(rv$dataset_list)
    selectInput("dataset", "Select Dataset:", choices = rv$dataset_list, selected = rv$current_dataset)
  })
#  output$existing_columns_ui <- renderUI({
  #    req(rv$existing_columns)
  #    selectInput("existing_column", "Show existing column:",
  #                choices = c("Select a column" = "", rv$existing_columns))
  #  })
...
#    cat("Rows in filtered:", ifelse(is.null(rv$filtered_data), 0, nrow(rv$filtered_data)), "\n")
  ...
  observeEvent(input$dataset,    { rv$current_dataset <- input$dataset; update_filtered_data() })
  ...
  # === File upload handler with UPSERT logic ===
  # Replace the ENTIRE observeEvent(input$upload_file, { ... }) block with this:
  ...
  observeEvent(input$upload_file, {
    ...
    uploaded_df <- tryCatch({
      ...
      # CONVERT ALL COLUMNS TO CHARACTER IMMEDIATELY
      for (col in names(df)) {
    ...
    # Preserve existing LABEIDs for rows where dataset+SampleID already exist
    ...
    ## Generate LABEIDs for NAs (as in your original)
    ...
    ## Align columns and ENSURE ALL DATA IS CHARACTER
    if (!is.null(rv$data) && nrow(rv$data)) {
      # Convert existing data to all character
    ...
    # — UPSERT on SampleID + dataset (NOT LABEID) —
    merged <- rv$data
    for (i in seq_len(nrow(uploaded_df))) {
      row_i <- uploaded_df[i, , drop=FALSE]
      
      # Match on SampleID + dataset only
      idx <- which(
      ...
      if (length(idx) > 0) {
        # Update existing row - preserve original LABEID if uploaded LABEID is "NA"
        if (row_i$LABEID == "NA") {
    ...
    updateSelectInput(session, "dataset", choices = rv$dataset_list, selected = new_ds)
    removeNotification(id)
  ...
  observeEvent(input$save, {
    ...
    tryCatch({
      ...
      showNotification(paste0("✔️ Wrote ", nrow(rv$data), " rows to:\n", file_path),
                       type="message", duration=5)
    }, error = function(e) {
