...
uploadDatasetServer <- function(id, ds, meta, go_to = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    showDatasetInfoModal <- function() {
      data_type_choices <- c(
        "RNAseq", "Single.Cell.RNA.seq", "Small.RNA.seq", "miRNA.seq", "Ribo.seq",
        "Microarray", "ChIP.seq", "ATAC.seq", "DNase.seq", "MNase.seq",
        "Mixture.Nuclease.seq", "Hi.C", "Capture.Hi.C", "3C", "4C", "5C",
        "Bisulfite.seq", "RRBS", "WGBS", "Mass.Spectrometry.Proteomics",
        "LC.MS.Proteomics", "SWATH.MS", "TMT.Quantitative.Proteomics",
        "SILAC.Quantitative.Proteomics", "Imaging.Mass.Spectrometry",
        "LC.MS.Metabolomics", "GC.MS.Metabolomics", "NMR.Metabolomics",
        "LC.MS.Lipidomics", "GC.MS.Lipidomics", "LC.MS.Glycomics",
        "Flow.Cytometry.Cytomics", "Single.Cell.Proteomics.MS",
        "Spatial.Transcriptomics", "Spatial.Proteomics", "Signal.Lipidomics"
      )
        ...
        fluidPage(
          ...
          fluidRow(
            column(
              ...
              selectInput(
                session$ns("data_type_input"), NULL,
            ...
            column(
              ...
              textInput(session$ns("author_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("fellow_input"), NULL, value = "")
            ),
            column(
              6,
              tags$label(
              ...
              textInput(session$ns("description_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("treatment_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("project_input"), NULL, value = "")
            ),
            column(
              6,
              tags$label(
              ...
              textInput(session$ns("invivo_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("sample_type_input"), NULL, value = "")
            ),
            column(
              6,
              tags$label(
              ...
              textInput(session$ns("date_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("samples_input"), NULL, value = "")
            ),
            column(
              6,
              tags$label(
              ...
              textInput(session$ns("conditions_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("replicates_input"), NULL, value = "")
            ),
            column(
              6,
              tags$label(
              ...
              textInput(session$ns("omics_location_input"), NULL, value = "")
            )
          ),
          fluidRow(
            column(
              ...
              textInput(session$ns("data_location_input"), NULL, value = "")
            ),
            column(
              6,
              tags$label(
              ...
              textInput(session$ns("publication_input"), NULL, value = "")
            )
          )
        ),
        footer = tagList(
          tags$div(class = "legend-required", "* Required fields"),
          modalButton("Cancel"),
          actionButton(session$ns("confirm_dataset_info"), "Continue to Metadata", 
    ...
    showPrefilledMetadataModal <- function() {
      ...
      showModal(modalDialog(
        ...
        fluidPage(
          ...
          fluidRow(
            column(
              ...
              numericInput(session$ns("meta_num_samples"), "Number of Samples", 
                           value = length(rv$extracted_samples), min = 1, max = 100)
            ),
            column(
              6, class = "hidden-field",
              textInput(session$ns("meta_dataset"), "Dataset", value = rv$filename_base)
            )
          ),
          fluidRow(
            column(
              6, class = "hidden-field",
              textInput(session$ns("meta_data_type"), "Data Type", value = rv$dataset_info$data_type)
            ),
            column(
              6, class = "hidden-field",
              textInput(session$ns("meta_author"), "Author", value = rv$dataset_info$author)
            )
          ),
          fluidRow(
            column(
              4,
              selectInput(session$ns("meta_show_column"), "Current Classifiers", 
                          choices = c("Select column" = "", rv$existing_columns), selectize = FALSE)
            ),
            column(
              4,
              textInput(session$ns("meta_default_value"), "Default Value", value = "NA")
            ),
            column(
              4,
              actionButton(session$ns("meta_add_existing_col"), "Add classifier", style = "margin-top:25px;")
            )
          ),
          fluidRow(
            column(
              4,
              textInput(session$ns("meta_new_col_name"), "New Classifier")
            ),
            column(
              4,
              textInput(session$ns("meta_new_col_default"), "Default Value", value = "NA")
            ),
            column(
              4,
              actionButton(session$ns("meta_add_new_col"), "Add new classifier", style = "margin-top:25px;")
            )
          ),
          DTOutput(session$ns("meta_preview_table")),
          tags$hr(),
        ...
        footer = tagList(
          modalButton("Cancel"),
          actionButton(session$ns("confirmDataUpload"), "Confirm", style = "background-color:#5cb85c; color:#fff;")
        )
      ))
      
      runjs(paste0(
        "$('#", session$ns("meta_num_samples"), "').prop('disabled', true);\n",
        "$('#", session$ns("meta_dataset"), "').prop('disabled', true);\n",
        "$('#", session$ns("meta_data_type"), "').prop('disabled', true);\n",
        "$('#", session$ns("meta_author"), "').prop('disabled', true);"
      ))
    ...
    output$meta_preview_table <- renderDT({
      ...
      if (rv$data_upload_mode) {
        ...
        tseng_col_idx <- which(names(rv$new_metadata) == "TsengID") - 1
    ...
    observeEvent(input$save, {
      ...
      tryCatch({
        ...
        showNotification(
          paste0("✔️ Wrote ", nrow(rv$data), " rows to metadata file"),
          type="message", duration=5
        )
      }, error = function(e) {
