  ...
  tagList(
    ...
    fluidRow(
      column(width = 3,
        ...
        box(title = "Reorder groups", status = "primary", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE, width = 12,
        ...
        box(title = "Labels", status = "primary", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE, width = 12,
            ...
            div(style = "max-height: 150px; overflow-y: auto;",
                ...
                sliderInput(ns("facet_font_size"), "Facet Gene Names Font Size:", min = 8, max = 24, value = 15, step = 1)
            )
        ),
        ...
        box(title = "Grid Controls", status = "primary", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE, width = 12,
        ...
        box(title = "Y axis scale", status = "primary", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE, width = 12,
            uiOutput(ns("yaxis_sliders"))
        ),
        box(title = "Aesthetic Settings", status = "primary", solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE, width = 12,
...
compareSamplesServer <- function(id, ds, meta, go_to = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    output$plot_ui <- renderUI({
      ...
      plotOutput(session$ns("facet_plot"),
    ...
    observe({
      req(ds())
      selected_metadata <- meta() %>% filter(dataset == ds())
      ...
      output$gene_select <- renderUI({
        selectizeInput(session$ns("genes"),"Select Genes", choices = NULL,
    ...
    observeEvent(input$conditions, {
      ...
      output$sortable_conditions <- renderUI({
        bucket_list(
          header = NULL,
          add_rank_list("Visible Groups",    labels = all_conds, input_id = session$ns("visible_conditions")),
          add_rank_list("Hidden Groups",     labels = NULL, input_id = session$ns("hidden_conditions"), class = "hidden-groups"),
    ...
    output$colorpicker_ui <- renderUI({
      ...
      tagList(lapply(seq_along(input$visible_conditions), function(i) {
        colourInput(inputId = session$ns(paste0("color_",i)),
    ...
    output$yaxis_sliders <- renderUI({
      ...
      tagList(
        lapply(seq_len(nrow(max_vals)), function(i) {
          ...
          mx   <- max_vals$maxE[i]
          sliderInput(session$ns(paste0("ymax_", gene)),
