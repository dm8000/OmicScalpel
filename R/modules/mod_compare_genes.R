...
compareGenesServer <- function(id, ds, meta, go_to = NULL, ai = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    find_expr_path <- function() {
      units <- list_units(ds())
      if (length(units) == 0) {
        stop("Nenhum arquivo válido foi encontrado para o dataset especificado.")
      }
      unit_reactive(units[1])
      expression_path(ds(), units[1])
    }
      ...
      output$sortable_conditions <- renderUI({
        bucket_list(
          ...
          add_rank_list("Visible Groups", labels   = all_conds,
                        input_id = session$ns("visible_conditions")),
          add_rank_list("Hidden Groups",  labels   = NULL,
                        input_id = session$ns("hidden_conditions"),
                        class    = "hidden-groups"),
    ...
    get_aesthetic_settings <- reactive({
      req(input$genes)
      cols <- sapply(seq_along(input$genes), function(i)
        input[[session$ns(paste0("color_", i))]])
      names(cols) <- input$genes
      list(
